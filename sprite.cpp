...
void CSprite::DrawLine(D3DXVECTOR3 start, D3DXVECTOR3 end, D3DXCOLOR color)
{
	...
	//ポリゴン描画
	CRenderer::GetDeviceContext()->Draw(2, 0);	//頂点数,offset
}
...
void CSprite::Draw(D3DXVECTOR2 pos, int width, int height)
{
	...
	//ポリゴン描画
	CRenderer::GetDeviceContext()->Draw(4, 0);	//頂点数,offset
...
void CSprite::Draw(D3DXMATRIX mtxWorld, int u, int v, int count, int delay)
{
	...
	float w = (1.0f / u);						//1枚当たりの幅
	float h = (1.0f / v);						//1枚当たりの高さ
	float x = (count / delay) % u * w;			//横座標
	float y = (count / delay) / u % v * h;		//縦座標
